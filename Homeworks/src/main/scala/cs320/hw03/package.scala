...
package object hw03 extends Homework03 {
  ...
  def run(str: String): String = {
    ...
    def interp(mrwfwae: MRFWAE, env: Env, rec: Rec): MRFWAEValue = mrwfwae match {
      ...
      case With(x, i, n) => {
//        print(s"WITH: When $x and $mrwfwae --> $env\n")
        interp(n, env + (x -> interp(i, env, rec)), rec)
      }
      case Id(x) => {
//        print(s"ID: When $x and $mrwfwae --> $env\n")
        env getOrElse(x, error(s"$x is a free identifier!"))
      }
      case App(f, a) => {
//        print(s"APP: When $f, $a and $mrwfwae --> $env\n")
        interp(f, env, rec) match {
        case CloV(x, b, fenv) => {
//          print(s"CLOV: $x $b $fenv\n")
          def tmp(mrwfwae_tmp: MRFWAE) = interp(mrwfwae_tmp, env, rec)

          def ListEnv(params: List[String], args: List[MRFWAEValue], env: Env): Env = {
          ...
          interp(b, fenv ++ ListEnv(x, a.map(tmp), fenv), rec)
        }
